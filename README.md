...
```
index.html      한 페이지 소개 사이트 (영상 → 문제 → 해법 → 시뮬레이터 → 알고리즘 → 하드웨어 → 검증 → 비교 → 로드맵 → 팀)
styles.css      전체 스타일 (모바일 우선 반응형, 다크 + 블루프린트 라이트 섹션)
app.js          내비게이션 · 스크롤 리빌 · 발표 영상 플레이어 · 적응 엔진 시뮬레이터
assets/         로고(SVG), 헬멧 설계도, Fusion 360 렌더, 발표 영상(MP4 18.6 MB)
